...
class Preprocessed_data:
    ...
    def preprocess_data(self):
        ...
        self.add_event()
        print("[OK] Preprocesamiento de datos completado")
    ...
    def check_input_data(self) -> bool:
        ...
        if missing_cols:
            print(f"[NOK] Faltan columnas requeridas: {missing_cols}")
            raise ValueError(f"Faltan columnas requeridas: {missing_cols}")

        # 4️⃣ Una sola fila (predicción online)
        if len(self.input_data) != 1:
            print(f"[NOK] Se esperaba 1 fila de entrada, se recibieron {len(self.input_data)}")
            raise ValueError(
        ...
        if self.input_data[list(required_columns)].isnull().any().any():
            print("[NOK] Existen valores nulos en columnas obligatorias")
        ...
        try:
            ...
            print("[NOK] La columna 'started_at' no es convertible a datetime")
        ...
        print("[OK] Datos de entrada correctos")
        
        return True

    def add_day_type(self):
        print("\t> Añadiendo la columna 'day_type'...")
    ...
    def add_timme_variables(self):
        print("\t> Añadiendo las columnas relacionadas con la fecha...")
    ...
    def add_event(self):
        ...
        """
        print("\t> Añadiendo la columna 'event'...")
        event_dates = {
